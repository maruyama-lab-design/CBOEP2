...
def evaluator(y_true, y_prob, out_keys: list=None, **kwargs):
    ...
    results["Balanced accuracy"] = metrics.balanced_accuracy_score(y_true, y_prob.round().astype(int))
    results["Specificity"] = metrics.recall_score(y_true, y_prob.round().astype(int), pos_label=0)
    # ___
